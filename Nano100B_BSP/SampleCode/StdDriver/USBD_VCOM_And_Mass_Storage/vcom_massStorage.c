...
{
    /* Bulk OUT */
    if (g_u32OutToggle == (USBD->EPSTS2 & USBD_EPSTS2_EPSTS6_Msk))
    {
        g_u32OutSkip = 1;
    ...
    {
        g_u8EP6Ready = 1;
        g_u32OutToggle = USBD->EPSTS2 & USBD_EPSTS2_EPSTS6_Msk;
        g_u32OutSkip = 0;
    }
}
...
void VCOM_LineCoding(uint8_t port)
{
    ...
    if (port == 0)
    {
        ...
        if(u32Baud_Div > 0xFFFF)
            UART0->BAUD = (UART_BAUD_MODE1 | UART_BAUD_MODE1_DIVIDER(__HXT, gLineCoding.u32DTERate));
        else
