 ...
 * @file     dfu_transfer.c
 * @version  V1.00
 * @brief    Nano100 series USBD DFU transfer sample file
 *
            ...
            case DFU_GETSTATUS:
            {
                if (dfu_status.bState == STATE_dfuDNLOAD_SYNC)
                {
                    ...
                    if (command_Count == 5)
                    {
                        ...
                        WriteData(prog_struct.block_num * TRANSFER_SIZE, (prog_struct.block_num * TRANSFER_SIZE) + prog_struct.data_len, (unsigned int *)prog_struct.buf);
